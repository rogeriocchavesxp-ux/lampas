...
CREATE INDEX IF NOT EXISTS knowledge_items_fts_idx ON public.knowledge_items USING gin(
  (
    ...
    setweight(to_tsvector('simple', array_to_string(authors, ' ')), 'A')
  )
);
  ...
  SELECT
    ...
    COALESCE(
      CASE WHEN v_tsq IS NOT NULL THEN ts_rank(
        ...
        setweight(to_tsvector('simple', array_to_string(k.tags, ' ')), 'C') ||
        setweight(to_tsvector('simple', array_to_string(k.authors, ' ')), 'A'),
        v_tsq
      ) ELSE 0 END
    , 0)::float4 AS rank
